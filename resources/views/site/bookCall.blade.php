<x-layout.mainlayout>
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 text-[#e0e1dd]">
    ...
    <!-- Dynamic JavaScript for 3-Step Wizard, Timezone, and Realtime Summary -->
    <script>
        // Global step controller
        ...
        function goToStep(step) {
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                if (i < step) {
                    // 1. Completed State: Clay bubble with checkmark icon
                    badge.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>';
                    badge.className = 'clay-step-bubble active text-sm font-black flex items-center justify-center transition-all duration-300';
                    if (sub) { sub.className = 'text-[10px] font-semibold uppercase tracking-wider text-emerald-400/90 block font-mono'; }
                    if (lbl) { lbl.className = 'text-xs sm:text-sm font-semibold text-white block mt-0.5'; }
                } else if (i === step) {
                    // 2. Active State: Prominent clay bubble with scale
                    badge.innerHTML = i;
                    badge.className = 'clay-step-bubble active text-sm font-black flex items-center justify-center scale-110 transition-all duration-300';
                    if (sub) { sub.className = 'text-[10px] font-semibold uppercase tracking-wider text-[#3fb950] block font-mono'; }
                    if (lbl) { lbl.className = 'text-xs sm:text-sm font-semibold text-white block mt-0.5'; }
                } else {
                    // 3. Upcoming State: Muted clay bubble
                    badge.innerHTML = i;
                    badge.className = 'clay-step-bubble text-sm font-medium flex items-center justify-center opacity-60 transition-all duration-300';
                    if (sub) { sub.className = 'text-[10px] font-semibold uppercase tracking-wider text-slate-500 block font-mono'; }
                    if (lbl) { lbl.className = 'text-xs sm:text-sm font-medium text-slate-400 block mt-0.5'; }
                }
            }

            // Update Summary Card Current Step Badge
            const currentStepBadge = document.getElementById('current-step-badge');
            if (currentStepBadge) {
                currentStepBadge.textContent = 'STEP ' + step + ' OF 3';
            }
        ...
        function validateStep2AndProceed() {
            ...
            if (!dateInput.value) {
                alert('Please pick a preferred date before proceeding.');
                dateInput.focus();
                return;
            }
            goToStep(3);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const tzBadge = document.getElementById('tz-badge');
            const dateInput = document.getElementById('booking-date');
            const slotInput = document.getElementById('selected-time-slot');
            const slotButtons = document.querySelectorAll('.slot-btn');
            const callCards = document.querySelectorAll('.call-type-card');
            const phoneReqBadge = document.getElementById('phone-req-badge');
            
            const summaryFormat = document.getElementById('summary-format');
            const summaryDate = document.getElementById('summary-date');
            const summarySlot = document.getElementById('summary-slot');
            const summaryTz = document.getElementById('summary-tz');

            // 1. Auto-detect visitor's timezone
            try {
                const detectedZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
                if (detectedZone) {
                    tzBadge.textContent = '📍 ' + detectedZone;
                    
                    // Check if detectedZone exists in options
                    let exists = false;
                    for (let i = 0; i < tzSelect.options.length; i++) {
                        if (tzSelect.options[i].value === detectedZone) {
                            exists = true;
                            if (!{{ old('timezone') ? 'true' : 'false' }}) {
                    ...
                    // If not found in default list, add it at the top
                    if (!exists && !{{ old('timezone') ? 'true' : 'false' }}) {
                        const newOption = new Option('📍 ' + detectedZone + ' (Your Detected Area)', detectedZone, true, true);
                        tzSelect.insertBefore(newOption, tzSelect.firstChild);
                    }
                }
            } catch (e) {
                tzBadge.textContent = 'Standard Time';
            }

            // 2. Format selection (Video vs Voice)
            callCards.forEach(card => {
                card.addEventListener('click', function() {
                    callCards.forEach(c => {
                        c.classList.remove('border-[#3fb950]', 'bg-[#0d1117]', 'shadow-[0_0_25px_rgba(63,185,80,0.18)]', 'scale-[1.01]');
                        c.classList.add('border-[#30363d]', 'bg-[#0d1117]/60');
                        const dot = c.querySelector('.call-check');
                        if (dot) dot.classList.add('hidden');
                    });

                    this.classList.remove('border-[#30363d]', 'bg-[#0d1117]/60');
                    this.classList.add('border-[#3fb950]', 'bg-[#0d1117]', 'shadow-[0_0_25px_rgba(63,185,80,0.18)]', 'scale-[1.01]');
                    const activeDot = this.querySelector('.call-check');
            ...
            slotButtons.forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    slotButtons.forEach(b => {
                        b.className = 'slot-btn text-xs font-semibold py-3 px-2 rounded-xl border transition-all text-center bg-[#0d1117] text-[#c9d1d9] border-[#30363d] hover:border-[#3fb950]/50 hover:text-white';
                    });

                    this.className = 'slot-btn text-xs font-bold py-3 px-2 rounded-xl border transition-all text-center bg-[#3fb950] text-[#0d1117] border-[#3fb950] shadow-[0_0_15px_rgba(63,185,80,0.3)] scale-[1.02]';
            ...
            if (tzSelect) tzSelect.addEventListener('change', updateSummary);

            // 5. Update summary helper
            function updateSummary() {
                // Call format
                const selectedRadio = document.querySelector('input[name="call_type"]:checked');
                const isVideo = selectedRadio ? selectedRadio.value === 'video' : true;
                if (isVideo) {
                    summaryFormat.innerHTML = '<span class="w-2 h-2 rounded-full bg-[#3fb950]"></span><span>Video Call (Google Meet)</span>';
                    if (phoneReqBadge) {
                        phoneReqBadge.textContent = '(Optional)';
                        phoneReqBadge.className = 'text-xs text-[#8a9e8a]';
                    }
                } else {
                    summaryFormat.innerHTML = '<span class="w-2 h-2 rounded-full bg-[#58a6ff]"></span><span>Voice Call (Phone/WhatsApp)</span>';
                    if (phoneReqBadge) {
                        phoneReqBadge.textContent = '(Required for call)';
                        phoneReqBadge.className = 'text-xs text-[#3fb950] font-bold';
                    }
                }
                ...
                if (dateInput && dateInput.value) {
                    try {
                        ...
                        summaryDate.textContent = d.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' });
                    } catch(e) {
                ...
                // Time slot
                if (slotInput) {
                    summarySlot.textContent = slotInput.value;
                }

                // Timezone
                if (tzSelect) {
                    const selectedText = tzSelect.options[tzSelect.selectedIndex] ? tzSelect.options[tzSelect.selectedIndex].text : tzSelect.value;
                    summaryTz.textContent = selectedText;
                }
            }

            // Initialize wizard to currentStep
