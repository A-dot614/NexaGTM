<x-app-layout>
    {{-- 
        NexaGTM Create Testimonial
        ─────────────────────────────
        Full-page form to add a new testimonial.
    --}}

    <x-slot name="title">Create Testimonial</x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <p class="text-xs text-[#8a9e8a] font-mono">Client reviews power social proof</p>
        </div>

        <div class="neomorph-card rounded-3xl p-6 sm:p-10 relative">
            <span class="style-tag style-tag-neomorph">New Testimonial</span>

            <h3 class="text-lg font-black text-white flex items-center gap-2 mb-1">
                <span class="text-[#3fb950]">✍️</span> Add a Testimonial
            </h3>
            <p class="text-xs text-[#8a9e8a] mb-8">Capture a new client review to showcase on the site.</p>

            <form method="POST" action="{{ route('dashboard.testimonials.store') }}" enctype="multipart/form-data" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="company" id="company" value="{{ old('company') }}" placeholder="e.g. Acme Inc."
                               class="w-full rounded-xl bg-[#0d1117] border border-[#30363d] px-4 py-2.5 text-sm text-white placeholder-[#8b949e] focus:border-[#3fb950] focus:ring-2 focus:ring-[#3fb950]/30 outline-none transition">
                    </div>
                    <div>
                        <label for="role" class="block text-xs font-bold uppercase tracking-wider text-[#8a9e8a] mb-1.5">Role</label>
                        <input type="text" name="role" id="role" value="{{ old('role') }}" placeholder="e.g. VP Sales"
                               class="w-full rounded-xl bg-[#0d1117] border border-[#30363d] px-4 py-2.5 text-sm text-white placeholder-[#8b949e] focus:border-[#3fb950] focus:ring-2 focus:ring-[#3fb950]/30 outline-none transition">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="5" required
                              class="w-full rounded-xl bg-[#0d1117] border border-[#30363d] px-4 py-2.5 text-sm text-white placeholder-[#8b949e] focus:border-[#3fb950] focus:ring-2 focus:ring-[#3fb950]/30 outline-none transition resize-y">{{ old('content') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="rating" id="rating"
                            ...
                            @for($opt = 5; $opt >= 1; $opt--)
                                <option value="{{ $opt }}" {{ old('rating', 5) == $opt ? 'selected' : '' }}>{{ str_repeat('★', $opt) . str_repeat('☆', 5 - $opt) }} ({{ $opt }})</option>
                            @endfor
                        </select>
                    </div>
                    <div>
